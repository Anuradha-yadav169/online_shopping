class OrdersController < ApplicationController
  ...
  def index
    ...
    @orders = if params[:search]
                Order.search(params[:search]).order('created_at ASC').paginate(page: params[:page], per_page: 5)
              else
                @orders.order('created_at ASC').paginate(page: params[:page], per_page: 5)
              end
  end
  ...
  def order_params
    params.require(:order).permit(:user_id, :description)
  end
