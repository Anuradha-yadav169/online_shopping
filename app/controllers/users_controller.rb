class UsersController < ApplicationController
  ...
  def index
    @users = if params[:search]
             User.search(params[:search]).order('created_at ASC').paginate(page: params[:page], per_page: 5)
            else
             User.where(activated: true).order('created_at ASC').paginate(page: params[:page], per_page: 5)
            end
  end
